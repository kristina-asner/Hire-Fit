...
export default function SignupPage() {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        setLoading(true);
        try {
            await axios.post('/api/auth/signup', { name, password });
            router.push('/');
            router.refresh();
        } catch (err) {
        ...
        <div className="min-h-screen bg-gray-50 text-gray-900 font-sans flex items-center justify-center px-6">
            <div className="w-full max-w-sm">
                <div className="flex flex-col items-center mb-8">
                    ...
                    <p className="text-sm text-gray-500 mt-1">Create your HR account</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-lg p-6 space-y-4">
                    ...
                    <div>
                        <label htmlFor="confirmPassword" className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5">
                            Confirm Password
                        </label>
                        <input
                            id="confirmPassword"
